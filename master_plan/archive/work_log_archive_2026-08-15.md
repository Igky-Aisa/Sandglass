...
Moved here by `sandglass rotate-logs` on 2026-08-15. Newest of these entries is immediately followed, chronologically, by the oldest entry still in the live file.
...
## 2026-08-14 - Claude (Sonnet 5) - 5% progress milestones + every stop reason now notifies, and a real brief-injection bug found along the way

### 1. Context Snapshot
- **Goal**: Add two things to the ntfy pipe: a push at every 5% prompt-throughput milestone (mirrored into `Progress.md`), and a push for every stop reason, not just some.
- **State**: `sandglass-cli/` — `prompt_source.py` (`throughput`), `project_docs.py` (`update_progress_throughput`), `storage.py` (`progress_notify_path`), `execution_engine.py` (`_maybe_notify_progress`, `_project_dir` fix, notify consolidation); new `tests/test_prompt_source.py`, `tests/test_project_docs.py`, `tests/test_progress_notify.py`.
- **Previous Blocker**: none carried over.

### 2. Work Done
- **Tracing "notify on every stop reason" surfaced a real, pre-existing double-fire.** `no_artifact` was notified once inline in `execute_queue` (at detection) and a second time by `run_with_auto_resume`'s post-hoc wrapper — confirmed by reading both call sites, not assumed. Separately, `error`/`left_in_place`/clean-success were notified *only* from the wrapper, meaning `--once` (which never reaches it) told the phone nothing for those three. Fixed by making every stop reason notify exactly once, inline in `execute_queue` at the point it's detected — the wrapper's redundant logic is deleted outright, not left dormant. Regression tests assert exact call sequences (not just "fired"), for both `execute_queue()` directly and through `run_with_auto_resume`.
- **The progress-milestone count is deliberately its own, simpler definition** (`prompt_source.throughput`) rather than reusing `templates/prompt_tools/count_blocks.py` — that script is project-side, opt-in, and tied to a `### P<phase>.<n>` heading convention some projects layer on top; this generic package can't assume it. It IS fence-aware, though, after reading count_blocks.py's own docstring: a completion entry legitimately quotes its block text inside a fenced block, and a naive `## ` count would double-count a quoted heading exactly like the four-day miscount that script documents.
- **`Progress.md`'s "Prompt throughput" section is now kept live automatically**, via `update_progress_throughput` — regex-matched to the exact three-bullet shape and nothing else, so it can never touch the qualitative "Against the main idea" judgment above it (a call only a human/agent can make) and returns `False` rather than guessing when the file doesn't exist or doesn't match.
- **Found and fixed a real, independent bug while building this**: `ExecutionEngine._project_dir` (used to locate a prompt's `master_plan/` for the brief, and now for this feature too) computed the project directory as a single `dirname()` on the queue source — landing on `prompt_tools/`, not the project root one level up. Confirmed empirically (a throwaway script, before touching any code) that `build_brief` returned `None` for the exact default layout — meaning the project-state brief has never actually fired for a project using `prompt_tools/future_prompts.md`, silently paying the re-read cost it exists to eliminate. Now walks up looking for `master_plan/`, bounded to a few levels. The artifact gate uses the same one-level `dirname()` pattern inline (not via this helper) but is *not* affected — it shells out to `git`, which resolves `--show-toplevel` itself regardless of `cwd`. Deliberately left that inline pattern untouched: it's safety-critical, confirmed sound, and a separate, higher-stakes change from anything else here.
- Milestone state (`.sandglass/progress_notify.json`, highest milestone already pushed) persists across restarts so a re-run doesn't re-announce 5%, 10%, 15%... from scratch.
